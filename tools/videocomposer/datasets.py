...
def random_resize(img, size):
    return TF.resize(
        img,
        size,
        interpolation=random.choice([InterpolationMode.BILINEAR, InterpolationMode.BICUBIC, InterpolationMode.LANCZOS]),
    )
...
class VideoDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        mask = (
            torch.from_numpy(cv2.resize(mask, (self.misc_size, self.misc_size), interpolation=cv2.INTER_NEAREST))
            .unsqueeze(0)
            .float()
        )
