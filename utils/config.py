...
class Config(object):
    ...
    def _parse_args(self):
        parser = argparse.ArgumentParser(description="Argparser for configuring [code base name to think of] codebase")
        parser.add_argument(
            "--cfg",
            dest="cfg_file",
    ...
    def _merge_cfg_from_command(self, args, cfg):
        ...
        for idx, key in enumerate(keys):
            ...
            assert key_split[0] in cfg.keys(), "Non-existant key: {}.".format(key_split[0])
            if len(key_split) == 2:
                assert key_split[1] in cfg[key_split[0]].keys(), "Non-existant key: {}.".format(key)
            elif len(key_split) == 3:
                assert key_split[1] in cfg[key_split[0]].keys(), "Non-existant key: {}.".format(key)
                assert key_split[2] in cfg[key_split[0]][key_split[1]].keys(), "Non-existant key: {}.".format(key)
            elif len(key_split) == 4:
                assert key_split[1] in cfg[key_split[0]].keys(), "Non-existant key: {}.".format(key)
                assert key_split[2] in cfg[key_split[0]][key_split[1]].keys(), "Non-existant key: {}.".format(key)
                assert (
                    key_split[3] in cfg[key_split[0]][key_split[1]][key_split[2]].keys()
                ), "Non-existant key: {}.".format(key)
            if len(key_split) == 1:
